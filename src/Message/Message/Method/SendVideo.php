 ...
 *
 * Typical usage: instantiate the method with the required 'video' field, then chain optional
...
class SendVideo extends Method implements MethodInterface {
    /**
     * Creates a new SendVideo method and sets the required 'video' field.
     *
     * @param string $video Video to send. Pass a file_id as String to send a video that exists on
     *   the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get a
     *   video from the Internet, or upload a new video using multipart/form-data. More information
     *   on Sending Files »
     */
    ...
    /**
     * Cover for the video in the message. Pass a file_id to send a file that exists on the Telegram
     ...
     *
     * @param string $cover Cover for the video in the message. Pass a file_id to send a file that
     *   exists on the Telegram servers (recommended), pass an HTTP URL for Telegram to get a file
