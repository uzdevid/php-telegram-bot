...
class SetWebhook extends Method implements MethodInterface {
    /**
     * Creates a new SetWebhook method and sets the required 'url' field.
     *
     * @param string $url HTTPS URL to send updates to. Use an empty string to remove webhook
     *   integration.
     */
     ...
     * (default). If not specified, the previous setting will be used. Please note that this
     * parameter doesn't affect updates created before the call to the setWebhook, so unwanted
     * updates may be received for a short period of time.
     *
     * @param array $allowedUpdates A JSON-serialized list of the update types you want your bot to
    ...
    /**
     * Adds one item to the allowed_updates list. A JSON-serialized list of the update types you
     * want your bot to receive. For example, specify ["message", "edited_channel_post",
     * "callback_query"] to only receive updates of these types. See Update for a complete list of
     * available update types. Specify an empty list to receive all update types except chat_member
     ...
     *
     * @param string $allowedUpdate Adds one item to the allowed_updates list. A JSON-serialized
     *   list of the update types you want your bot to receive. For example, specify ["message",
     *   "edited_channel_post", "callback_query"] to only receive updates of these types. See Update
     *   for a complete list of available update types. Specify an empty list to receive all update
     *   types except chat_member , message_reaction , and message_reaction_count (default). If not
    ...
    public function addAllowedUpdate(string $allowedUpdate): static {
        $this->pushAttribute('allowed_updates', $allowedUpdate);
        return $this;
    }
     ...
     *
     * @param string $secretToken A secret token to be sent in a header
